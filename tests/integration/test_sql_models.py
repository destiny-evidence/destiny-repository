...
async def test_reference_get_with_duplicates(session: AsyncSession):
    ...
    # 1. Create references
    ref_a = Reference(id=uuid.uuid7(), visibility=Visibility.PUBLIC)
    ref_b = Reference(id=uuid.uuid7(), visibility=Visibility.PUBLIC)
    ref_c = Reference(id=uuid.uuid7(), visibility=Visibility.PUBLIC)
    ...
    with pytest.raises(SQLIntegrityError):
        await dup_repo.add(a_is_canonical.model_copy(update={"id": uuid.uuid7()}))
