...
from pydantic import UUID4, UUID7, SecretStr
...
class RobotService(GenericService[RobotAntiCorruptionService]):
    ...
    async def get_robot(self, robot_id: UUID4 | UUID7) -> Robot:
    ...
    @sql_unit_of_work
    async def get_robot_standalone(self, robot_id: UUID4 | UUID7) -> Robot:
        """Return a given robot."""
        return await self.get_robot(robot_id)

    async def get_robot_secret(self, robot_id: UUID4 | UUID7) -> str:
    ...
    @sql_unit_of_work
    async def get_robot_secret_standalone(self, robot_id: UUID4 | UUID7) -> str:
        """Return secret used for signing requests sent to this robot."""
    ...
    @sql_unit_of_work
    async def cycle_robot_secret(self, robot_id: UUID4 | UUID7) -> Robot:
