...
class ImportService(GenericService[ImportAntiCorruptionService]):
    ...
    async def _get_import_record(self, import_record_id: UUID4 | UUID7) -> ImportRecord:
        """Get a single import by id."""
        return await self.sql_uow.imports.get_by_pk(import_record_id)

    @sql_unit_of_work
    async def get_import_record(self, import_record_id: UUID4 | UUID7) -> ImportRecord:
        """Get a single import by id."""
        return await self._get_import_record(import_record_id)

    @sql_unit_of_work
    async def get_import_record_with_batches(self, pk: UUID4 | UUID7) -> ImportRecord:
    ...
    @sql_unit_of_work
    async def get_import_batch(self, import_batch_id: UUID4 | UUID7) -> ImportBatch:
    ...
    async def get_imported_references_from_batch(
        self, import_batch_id: UUID4 | UUID7
    ) -> set[UUID4 | UUID7]:
    ...
    async def update_import_result(
        self, import_result_id: UUID4 | UUID7, **kwargs: object
    ) -> ImportResult:
...
"""
    ...
    @sql_unit_of_work
    async def finalise_record(self, import_record_id: UUID4 | UUID7) -> None:
