...
class ReferenceDownloadStatus(StrEnum):
    ...
    PENDING = auto()
    """The download job has been queued but not yet started."""
    RUNNING = auto()
    """The download job is currently being processed."""
    COMPLETED = auto()
    """The download job completed and the file is available."""
    FAILED = auto()
    """The download job failed before producing a file."""


class ReferenceDownloadRead(BaseModel):
    """A reference download job, used to poll for status and a signed URL."""

    id: UUID = Field(description="The ID of the download job.")
    ...
    result_url: HttpUrl | None = Field(
        ...
        description=(
            ...
            "``status`` is ``completed``; if the URL expires, a new one can be "
            "generated by calling ``GET /references/download/{id}/`` again."
        ),
    )
