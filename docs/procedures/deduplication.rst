...
The core invariant is that **only canonicals can have duplicates, and duplicates can only point to canonicals**. There is no chaining of duplicates.
...
3. The reference being marked as a duplicate already has duplicates pointing to it (which would violate the no-chaining invariant).
